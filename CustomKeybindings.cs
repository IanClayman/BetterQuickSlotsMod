...
public class CustomKeybindings {
    ...
    // The player-specific RewiredInputs instances are in a private variable, and will be accessed on Update(),
    // so it exposed and cached here for convenience & performance
    public static Dictionary<int, RewiredInputs> m_playerInputManager;
    ...
    //
    /// <summary>
    /// Creates a new Action and Keybinding to be used in-game
    /// </summary>
    /// <param name="name">String displayed in the keybidings menu</param>
    /// <param name="categoryId">KeyBindingsCategory that determines what type of action this is, and what submenu it goes under in the keybindings</param>
    /// <param name="showForController">If true, this action will also appear in the controller keybinding menu</param>
    /// <param name="sectionId">Has a role in determining how far down in the submenu list the new action appears.  Play around with the value until happy [0-5 inclusive]</param>
    /// <param name="type">Choice between InputActionType.Button (i.e. AlphaNum on keyboard, ABXY on controller) or InputActionType.Axis (arrow keys on keyboard, Joystick on controller)</param>
    public static void AddAction(string name, KeybindingsCategory categoryId, bool showForController = false, int sectionId = 0, InputActionType type = InputActionType.Button) {
        ...
        InputActionDescription protoAction = new InputActionDescription(name, (int)categoryId, showForController, sectionId, type);
